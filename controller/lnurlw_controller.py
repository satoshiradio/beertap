...
class LnurlwController:
    ...
    def on_lnurlw(self, lnurlw: str):
        logging.info('LnurlwController.on_lnurlw')
        try:
            logging.info('sending lnurlw get request 1 to:', lnurlw)
            resp1 = requests.get(lnurlw)
            json1 = resp1.json()
            logging.info('LnurlwController.on_lnurlw: got response1:', json1)
            ...
            if status1 == 'ERROR':
                ...
                logging.error('LnurlwController.on_lnurlw: Server returned error:', reason1)
                return

            if callback is None or not isinstance(callback, str) or k1 is None or not isinstance(k1, str):
                logging.error('LnurlwController.on_lnurlw: Server response invalid:', json1)
            ...
            logging.info('sending lnurlw get request 1 to:', url)
            resp2 = requests.get(url)
            json2 = resp2.json()
            logging.info('LnurlwController.on_lnurlw: got response2:', json2)
            ...
            if status2 == 'ERROR':
                ...
                logging.error('LnurlwController.on_lnurlw: Server returned error reason:', reason2)

            logging.error('server sent invalid response. doing nothing.', json2)
        except Exception as e:
            logging.error('LnurlwController.on_lnurlw error:', e)
